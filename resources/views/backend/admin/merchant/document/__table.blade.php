<div class="table-responsive table-nowrap d-box p-0 border-radius-0">
    <table class="table">
        ...
        @forelse($documents as $document)
            <tr>
                <td>{{ $i++ }}</td>
                <td>{{ !empty($document->merchant) ? $document->merchant->name : \App\Helpers\GeneralHelper::EMPTY_DASHES() }}</td>
                <td>{{ \App\Helpers\GeneralHelper::FORMAT_DATE($document->created_at) ?? \App\Helpers\GeneralHelper::EMPTY_DASHES() }}</td>
                <td>{{ !empty($document->merchant->email) ? $document->merchant->email : \App\Helpers\GeneralHelper::EMPTY_DASHES() }}</td>
                <td>{{ !empty($document->merchant->mobile_number) ? $document->merchant->mobile_number : \App\Helpers\GeneralHelper::EMPTY_DASHES() }}</td>
                <td>
                    <span
